...
!      Boussinesq type-------------------------------------------------
     ...
     do i=0,mx+1
        do j=0,my+1
           hbar_tt(i,j)=-((aux(1,i,j)-aux(4,i,j))/dt-(aux(4,i,j)-aux(5,i,j))/dt1)/dt/2.d0
        end do
     end do

     do i=1,mx
        do j=1,my
           hbar_xtt(i,j) = (hbar_tt(i+1,j)-hbar_tt(i-1,j))/2.d0/dx
           hbar_ytt(i,j) = (hbar_tt(i,j+1)-hbar_tt(i,j-1))/2.d0/dy
        end do
     end do 
     endif
...
!======================================================================
     ...
     do j=1,my
        do i=1,mx
           ...
           elseif (maxval(slope(i-2:i+2,j-2:j+2))>-1.d-1 ) then
